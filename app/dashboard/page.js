...
export default function DashboardPage() {
  ...
  return (
    <div className="dashboard-layout" style={{ backgroundColor: '#0f172a', minHeight: '100vh', display: 'grid', gridTemplateColumns: '260px 1fr', color: '#f8fafc', fontFamily: 'system-ui, sans-serif' }}>
      ...
      {/* Контент вкладок через компоненты */}
      <main style={{ padding: '32px', overflowY: 'auto' }}>
        {activeTab === 'profile' && (
          <ProfileTab user={user} setUser={setUser} handleUpdateProfile={handleUpdateProfile} inputStyle={inputStyle} labelStyle={labelStyle} btnPrimary={btnPrimary} />
        )}
        {activeTab === 'students' && (
          <StudentsTab students={students} setShowAddStudentModal={setShowAddStudentModal} handleDeleteStudent={handleDeleteStudent} cardStyle={cardStyle} btnPrimary={btnPrimary} btnSmallBlue={btnSmallBlue} btnSmallDanger={btnSmallDanger} />
        )}
        {activeTab === 'tests' && (
          <TestsTab exams={exams} setRegisterModal={setRegisterModal} cardStyle={cardStyle} btnPrimary={btnPrimary} />
        )}
        {activeTab === 'bookings' && (
          <BookingsTab bookings={bookings} openTicketFromBooking={openTicketFromBooking} cardStyle={cardStyle} btnPrimary={btnPrimary} btnSmallBlue={btnSmallBlue} />
        )}
      </main>
          ...
          <div style={modalContent}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <button onClick={handlePayment} style={{ ...btnPrimary, backgroundColor: '#16a34a', color: '#fff', marginTop: '10px', width: '100%', justifyContent: 'center' }}>
                ⚡ Төлемді растау және кіру қағазын алу
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={modalOverlay}>
          <div style={{ ...modalContent, textAlign: 'center', maxWidth: '440px', padding: '20px' }}>
            ...
            <div style={{ backgroundColor: '#0f172a', padding: '14px', borderRadius: '12px', border: '1px solid #334155', textAlign: 'left', marginBottom: '14px', fontSize: '13px' }}>
              ...
              <p style={{ margin: '4px 0', color: '#cbd5e1' }}><strong>Мекен-жайы:</strong> <span style={{color:'#fff'}}>{ticketModal.classroom}</span></p>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '12px', backgroundColor: '#1e293b', padding: '10px', borderRadius: '8px' }}>
                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(ticketModal.uniqueCode)}`} alt="QR Code" style={{ width: '95px', height: '95px', borderRadius: '6px', background: '#fff', padding: '4px' }} />
                <span style={{ color: '#38bdf8', fontWeight: '800', fontSize: '14px', letterSpacing: '1px', marginTop: '6px' }}>{ticketModal.uniqueCode}</span>
              </div>
            </div>
