...
export default function AdminPage() {
  ...
  if (!isAuthenticated) {
    return (
      <div style={{ backgroundColor: '#f1f5f9', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui, sans-serif', padding: '16px' }}>
        <form onSubmit={handleLogin} style={{ backgroundColor: '#ffffff', padding: '30px 20px', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', width: '100%', maxWidth: '380px' }}>
  ...
  return (
    <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh', display: 'flex', flexDirection: 'column', color: '#0f172a', fontFamily: 'system-ui, sans-serif' }}>
      
      {/* Мобильный верхний бар */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', backgroundColor: '#ffffff', borderBottom: '1px solid #e2e8f0' }}>
        <div style={{ fontSize: '20px', fontWeight: '900', color: '#0284c7' }}>
          QUQU<span style={{ color: '#f43f5e' }}>.</span> admin
        </div>
        <button 
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} 
          style={{ backgroundColor: '#0284c7', color: '#fff', border: 'none', padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>
          {isMobileMenuOpen ? '✕ Жабу' : '☰ Мәзір'}
        </button>
      </div>

      <div style={{ display: 'flex', flex: 1, position: 'relative', flexDirection: 'row' }}>
        {/* Боковое меню (на ПК всегда видно, на мобилках открывается по кнопке) */}
        <aside style={{ 
          width: '260px', 
          ...
          height: '100%',
          zIndex: 100,
          boxShadow: isMobileMenuOpen ? '5px 0 15px rgba(0,0,0,0.1)' : 'none',
          '@media (max-width: 768px)': {
            display: isMobileMenuOpen ? 'flex' : 'none'
          }
        }}>
          <div>
            <div style={{ fontSize: '22px', fontWeight: '900', color: '#0284c7', marginBottom: '32px', display: 'none' /* Скрываем дубль на мобилках */ }}>
              QUQU<span style={{ color: '#f43f5e' }}>.</span> admin
            </div>
            <nav style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <button onClick={() => { setActiveTab('users'); setIsMobileMenuOpen(false); }} style={menuBtn(activeTab === 'users')}>👤 Пайдаланушылар</button>
              <button onClick={() => { setActiveTab('students'); setIsMobileMenuOpen(false); }} style={menuBtn(activeTab === 'students')}>🎓 Оқушылар</button>
              <button onClick={() => { setActiveTab('tests'); setIsMobileMenuOpen(false); }} style={menuBtn(activeTab === 'tests')}>📝 Тесттер</button>
              <button onClick={() => { setActiveTab('results'); setIsMobileMenuOpen(false); }} style={menuBtn(activeTab === 'results')}>📊 Нәтижелер</button>
            </nav>
          </div>
        ...
        {/* Основной контент */}
        <main style={{ flex: 1, padding: '20px', overflowY: 'auto', width: '100%', boxSizing: 'border-box' }}>
          {activeTab === 'tests' && (
            <div>
            ...
            <div>
              ...
              <div style={tableContainer}>
                <table style={tableStyle}>
                  ...
                  <tbody>
                    {users.length === 0 ? (
                      <tr><td colSpan="3" style={tdStyle}>Деректер жоқ</td></tr>
                    ) : (
                      users.map((u) => (
          ...
          {activeTab === 'students' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
                ...
                <button onClick={() => exportToExcel(students, 'students_list')} style={btnGreen}>📥 Excel</button>
              </div>
              <div style={tableContainer}>
                <table style={tableStyle}>
                  <thead>
                    <tr style={thTr}>
                      <th style={thStyle}>Аты-жөні</th>
                      <th style={thStyle}>ЖСН</th>
                      <th style={thStyle}>Мектеп</th>
                    </tr>
                  </thead>
                  <tbody>
                    {students.length === 0 ? (
                      <tr><td colSpan="3" style={tdStyle}>Оқушылар әлі енгізілмеген</td></tr>
                    ) : (
                      students.map((s) => (
                  ...
                  <tbody>
                    {tests.map((t) => (
                      <tr key={t.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={tdStyle}>
                          <label style={{ ...btnBlueUpload, padding: '6px 10px', fontSize: '12px' }}>
                            📤 Жүктеу
...
const pageTitle = { margin: 0, fontSize: '20px', fontWeight: '800', color: '#0f172a' };
...
const tableStyle = { width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '14px', minWidth: '300px' };
const thTr = { backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0' };
const thStyle = { padding: '12px 14px', color: '#64748b', fontWeight: '700', fontSize: '11px', textTransform: 'uppercase' };
const tdStyle = { padding: '14px', color: '#334155' };
...
const btnBlueUpload = { backgroundColor: '#0284c7', color: '#fff', padding: '8px 14px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', display: 'inline-block', fontSize: '13px', textAlign: 'center' };
...
const btnSmallDanger = { backgroundColor: '#ef4444', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', fontSize: '12px' };
const btnGreen = { backgroundColor: '#16a34a', color: '#fff', border: 'none', padding: '8px 14px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px' };
